...
extern int pqcrystals_kyber512_90s_ref_dec_measure(uint8_t *ss, const uint8_t *ct, const uint8_t *sk, uint64_t *rdtsc_buffer, const unsigned char **rdtsc_labels, int *fail);
...
extern int pqcrystals_kyber512_90s_avx2_dec_measure(uint8_t *ss, const uint8_t *ct, const uint8_t *sk, uint64_t *rdtsc_buffer, const unsigned char **rdtsc_labels, int *fail);
#endif
...
OQS_API OQS_STATUS OQS_KEM_kyber_512_90s_decaps_measure(uint8_t *shared_secret, const uint8_t *ciphertext, const uint8_t *secret_key, uint64_t *rdtsc_buffer, const unsigned char **rdtsc_labels, int *fail) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AES) && OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT)) {
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS) pqcrystals_kyber512_90s_avx2_dec_measure(shared_secret, ciphertext, secret_key, rdtsc_buffer, rdtsc_labels, fail);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) pqcrystals_kyber512_90s_ref_dec_measure(shared_secret, ciphertext, secret_key, rdtsc_buffer, rdtsc_labels, fail);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) pqcrystals_kyber512_90s_ref_dec_measure(shared_secret, ciphertext, secret_key, rdtsc_buffer, rdtsc_labels, fail);
