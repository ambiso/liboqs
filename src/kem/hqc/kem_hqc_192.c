...
extern int PQCLEAN_HQCRMRS192_CLEAN_crypto_kem_enc_with_m(uint8_t *ct, uint8_t *ss, const uint8_t *pk, const uint8_t *m);
extern int PQCLEAN_HQCRMRS192_CLEAN_crypto_kem_numrejections(uint8_t *plaintext, size_t* seed_expanded_bytes);
...
extern int PQCLEAN_HQCRMRS192_AVX2_crypto_kem_enc_with_m(uint8_t *ct, uint8_t *ss, const uint8_t *pk, const uint8_t *m);
extern int PQCLEAN_HQCRMRS192_AVX2_crypto_kem_numrejections(uint8_t *plaintext, size_t* seed_expanded_bytes);
...
OQS_API OQS_STATUS OQS_KEM_hqc_192_encaps_with_m(uint8_t *ciphertext, uint8_t *shared_secret, const uint8_t *public_key, const uint8_t *m) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI1) && OQS_CPU_has_extension(OQS_CPU_EXT_PCLMULQDQ)) {
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS) PQCLEAN_HQCRMRS192_AVX2_crypto_kem_enc_with_m(ciphertext, shared_secret, public_key, m);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) PQCLEAN_HQCRMRS192_CLEAN_crypto_kem_enc_with_m(ciphertext, shared_secret, public_key, m);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) PQCLEAN_HQCRMRS192_CLEAN_crypto_kem_enc_with_m(ciphertext, shared_secret, public_key, m);
...
OQS_API OQS_STATUS OQS_KEM_hqc_192_numrejections(uint8_t *plaintext, size_t *seed_expanded_bytes) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI1) && OQS_CPU_has_extension(OQS_CPU_EXT_PCLMULQDQ)) {
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS) PQCLEAN_HQCRMRS192_AVX2_crypto_kem_numrejections(plaintext, seed_expanded_bytes);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) PQCLEAN_HQCRMRS192_CLEAN_crypto_kem_numrejections(plaintext, seed_expanded_bytes);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) PQCLEAN_HQCRMRS192_CLEAN_crypto_kem_numrejections(plaintext, seed_expanded_bytes);
