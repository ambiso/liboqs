...
void PQCLEAN_HQCRMRS192_CLEAN_hqc_pke_decrypt_intermediates(uint8_t *m, uint8_t *rmencoded, uint8_t *rmdecoded, const uint64_t *u, const uint64_t *v, const unsigned char *sk) {
    ...
    PQCLEAN_HQCRMRS192_CLEAN_reed_muller_decode(tmpdecode, (uint8_t *)tmp1);
    memcpy(rmdecoded, tmpdecode, sizeof(tmpdecode));
    PQCLEAN_HQCRMRS192_CLEAN_reed_solomon_decode(m, tmpdecode);

    PQCLEAN_HQCRMRS192_CLEAN_reed_solomon_encode(tmpencode, m);
...
void PQCLEAN_HQCRMRS192_CLEAN_hqc_pke_eprime(uint8_t *eprime, uint8_t *m, const uint64_t *u, const uint64_t *v, const unsigned char *sk) {
    ...
    // Compute m.G
    PQCLEAN_HQCRMRS192_CLEAN_code_encode((uint8_t *)tmp1, m);
    PQCLEAN_HQCRMRS192_CLEAN_load8_arr(tmp1, VEC_N1N2_SIZE_64, (uint8_t *)tmp1, VEC_N1N2_SIZE_BYTES);
    PQCLEAN_HQCRMRS192_CLEAN_vect_resize(tmp1, PARAM_N, tmp1, PARAM_N1N2);

    // Compute e' = v - u.y + m.G
    PQCLEAN_HQCRMRS192_CLEAN_vect_add(tmp2, tmp1, tmp2, VEC_N_SIZE_64);
    PQCLEAN_HQCRMRS192_CLEAN_store8_arr(eprime, VEC_N1N2_SIZE_BYTES, tmp2, VEC_N_SIZE_64);
}