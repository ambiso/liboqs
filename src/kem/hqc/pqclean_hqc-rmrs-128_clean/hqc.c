...
void PQCLEAN_HQCRMRS128_CLEAN_hqc_pke_keygen(unsigned char *pk, unsigned char *sk) {
    ...
    // Compute secret key
    PQCLEAN_HQCRMRS128_CLEAN_vect_set_random_fixed_weight(NULL, &sk_seedexpander, x, PARAM_OMEGA);
    PQCLEAN_HQCRMRS128_CLEAN_vect_set_random_fixed_weight_by_coordinates(NULL, &sk_seedexpander, y, PARAM_OMEGA);
...
void PQCLEAN_HQCRMRS128_CLEAN_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint8_t *m, unsigned char *theta, const unsigned char *pk) {
    ...
    PQCLEAN_HQCRMRS128_CLEAN_vect_set_random_fixed_weight(NULL, &seedexpander, r1, PARAM_OMEGA_R);
    PQCLEAN_HQCRMRS128_CLEAN_vect_set_random_fixed_weight_by_coordinates(NULL, &seedexpander, r2, PARAM_OMEGA_R);
    PQCLEAN_HQCRMRS128_CLEAN_vect_set_random_fixed_weight(NULL, &seedexpander, e, PARAM_OMEGA_E);
...
void PQCLEAN_HQCRMRS128_CLEAN_hqc_pke_error_components(const unsigned char *m, uint64_t *r1, uint64_t *r2, uint64_t *e) {
    ...
    // Generate r1, r2 and e
    PQCLEAN_HQCRMRS128_CLEAN_vect_set_random_fixed_weight(NULL, &seedexpander, r1, PARAM_OMEGA_R);
    PQCLEAN_HQCRMRS128_CLEAN_vect_set_random_fixed_weight(NULL, &seedexpander, r2, PARAM_OMEGA_R);
    PQCLEAN_HQCRMRS128_CLEAN_vect_set_random_fixed_weight(NULL, &seedexpander, e, PARAM_OMEGA_E);
}
...
void PQCLEAN_HQCRMRS128_CLEAN_hqc_pke_eprime(uint8_t *eprime, uint8_t *m, const uint64_t *u, const uint64_t *v, const unsigned char *sk) {
    ...
    // Compute m.G
    PQCLEAN_HQCRMRS128_CLEAN_code_encode((uint8_t *)tmp1, m);
    PQCLEAN_HQCRMRS128_CLEAN_load8_arr(tmp1, VEC_N1N2_SIZE_64, (uint8_t *)tmp1, VEC_N1N2_SIZE_BYTES);
    PQCLEAN_HQCRMRS128_CLEAN_vect_resize(tmp1, PARAM_N, tmp1, PARAM_N1N2);

    // Compute e' = v - u.y + m.G
    PQCLEAN_HQCRMRS128_CLEAN_vect_add(tmp2, tmp1, tmp2, VEC_N_SIZE_64);
    PQCLEAN_HQCRMRS128_CLEAN_store8_arr(eprime, VEC_N1N2_SIZE_BYTES, tmp2, VEC_N_SIZE_64);
}
