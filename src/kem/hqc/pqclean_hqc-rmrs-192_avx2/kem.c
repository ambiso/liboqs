 ...
 * @returns 0 if encapsulation is successful
 */
int PQCLEAN_HQCRMRS192_AVX2_crypto_kem_enc_with_m(unsigned char *ct, unsigned char *ss, const unsigned char *pk, const unsigned char *m) {
    ...
    // Encrypting m
    PQCLEAN_HQCRMRS192_AVX2_hqc_pke_encrypt(u, v, (uint8_t*)m, theta, pk);
...
int PQCLEAN_HQCRMRS192_AVX2_crypto_kem_numrejections(uint8_t m[VEC_K_SIZE_BYTES], size_t *seed_expanded_bytes) {
    ...
    PQCLEAN_HQCRMRS192_AVX2_vect_set_random_fixed_weight(&trace, &seedexpander, r1, PARAM_OMEGA_R);
    PQCLEAN_HQCRMRS192_AVX2_vect_set_random_fixed_weight(&trace, &seedexpander, r2, PARAM_OMEGA_R);
    PQCLEAN_HQCRMRS192_AVX2_vect_set_random_fixed_weight(&trace, &seedexpander, e, PARAM_OMEGA_E);
